%%This code is for Fig 6.6 for senstive of different \rho
...
f1=(U(i+1,j,n)-2*U(i,j,n)+U(i-1,j,n))/(ds)^2; %% ƫS^2
f2=(U(i+1,j+1,n)+U(i-1,j-1,n)-U(i-1,j+1,n)-U(i+1,j-1,n))/(4*ds*dv);  %% SV
f3=(U(i,j+1,n)-2*U(i,j,n)+U(i,j-1,n))/(dv)^2; %% ƫV^2
...
plot(rho1,price,'-*');
title(' Option  price  for different \rho','fontsize',16,'FontWeight','bold')
